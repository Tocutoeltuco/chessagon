...
use self::p2p::Connection;
use self::packet::Packet;
use self::signal::SignalClient;
...
pub struct Client {
    ...
    onmessage: Option<Box<dyn FnMut(&Connection, Packet)>>,
...
impl Client {
    ...
    pub fn set_onmessage(&mut self, handler: Box<dyn FnMut(&Connection, Packet)>) {
